...
async def group(client, message):
    ...
    if 2 < len(message.text) < 50:    
        ...
        if files:
            ...
            await message.reply_text(text=f'Samahani {searchi} uliyotafta haipo kwenye database zetu.Tafadhali bonyeza Button kisha ukurasa unaofuata ntumie jina la movie au series ntakupa jibu kwa haraka iwezekanavyo ',reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text='ADMIN',user_id=int(user_id3))]]))
        else:
            return
        if not btn:
            return
