...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if (clicked == typed):
        if query.data == "kundii":
            ...
            if mkv.photo:
                await query.message.delete()
                txt = f'{query.from_user.id} {query.data.split(" ")[1]}'
