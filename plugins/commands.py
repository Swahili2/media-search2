...
async def new_filtervip(client: Client, message):
    ...
    try:
        ...
        return
    mkv = await client.ask(text=f'tafadhal naomba utume bei(namba tu) ya  {usr[ab2-1]} hii kama ni bure tuma neno free mfano 500. (Kumbuka Hamna bei 0)',chat_id = message.from_user.id)
    try:
        ab1=int(mkv.text)
        if ab1==0:
